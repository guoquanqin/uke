            ...
            <div class="contentleft">
                ...
                <div class="order">
                    <div class="orderTop">
                        ...
                        <a href="${pageContext.request.contextPath}/management/orderSearchForRecevied.action?pageNum=1" style="border-bottom:1px solid green;color:green;">已接收订单</a>
                        ...
                        <span>${today }</span>
                        <c:if test="${size!=0 }">
                        	<button id="delivery" class="btn btn-info">一键配送</button>
                        </c:if>
                    </div>
                    <!-- 如果有订单 -->
                    <c:if test="${size!=0 }">
                        ...
                        <tbody>
                       	<c:forEach items="${orderUserListForPage }" var="i" varStatus="status">
                           	...
                           	<c:if test="${i.isExit!='2' }">
	                            ...
	                            <td>
	                            	<c:if test="${i.isExit=='2' }">
		                                <span><a href="${pageContext.request.contextPath}/management/deliveryIntegralOrder.action?id=${i.orderUserId}">配送</a></span>
		                          	</c:if>
		                          	<c:if test="${i.isExit!='2' }">
		                                <span><a href="${pageContext.request.contextPath}/management/deliveryOrder.action?id=${i.orderUserId}">配送</a></span>
			                        </c:if>
	                            </td>
		...
		$.ajax({
			...
			success:function(data){
				if(data=='1'){
					...
					window.location.href="${pageContext.request.contextPath}/management/orderSearchForRecevied.action?pageNum=1";
					console.log(data);
				}
				getTimeOrder();
			}
