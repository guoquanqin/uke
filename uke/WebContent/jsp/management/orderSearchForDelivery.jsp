            ...
            <div class="contentleft">
                ...
                <div class="order">
                    <div class="orderTop">
                        ...
                        <a style="border-bottom:1px solid green;color:green;" href="${pageContext.request.contextPath}/management/orderSearchForDelivery.action?pageNum=1">正在配送</a>
                        <a href="${pageContext.request.contextPath}/management/orderSearchForFinish.action?pageNum=1">已收货</a>
                        <span>${today }</span>
                        <c:if test="${size!=0 }">
                        	<button id="finish" class="btn btn-info">一键送达</button>
                        </c:if>
                    </div>
                    <!-- 如果有订单 -->
                    <c:if test="${size!=0 }">
                        ...
                        <tbody>
                       	<c:forEach items="${orderUserListForPage }" var="i" varStatus="status">
                           	...
                           	<c:if test="${i.isExit!='2' }">
	                            ...
	                            <td>
	                            	<c:if test="${i.isExit=='2' }">
		                                <span><a href="${pageContext.request.contextPath}/management/finishIntegralOrder.action?id=${i.orderUserId}">送达</a></span>
		                          	</c:if>
		                          	<c:if test="${i.isExit!='2' }">
		                                <span><a href="${pageContext.request.contextPath}/management/finishOrder.action?id=${i.orderUserId}">送达</a></span>
			                        </c:if>
	                            </td>
...
<script>
		...
		$.ajax({
			...
			success:function(data){
				if(data=='1'){
					...
					window.location.href="${pageContext.request.contextPath}/management/orderSearchForDelivery.action?pageNum=1";
					console.log(data);
				}
				getTimeOrder();
			}
    ...
    $(function () {
        ...
        $("#finish").click(function (e) {
			  window.location.href="${pageContext.request.contextPath}/management/finishAllOrder.action";
         });
