...
<%-- <meta http-equiv="refresh" content="3;url=${pageContext.request.contextPath}/management/orderSearch.action?pageNum=1"> --%>
<title>订单详情-新订单</title>
    ...
    <div class="mainContent">
        <div class="content">
            <div class="contentleft">
                ...
                <div class="order">
                    <div class="orderTop">
                        <a  style="border-bottom:1px solid green;color:green;" href="${pageContext.request.contextPath}/management/orderSearch.action?pageNum=1">新订单</a>
                        ...
                        <a href="${pageContext.request.contextPath}/management/orderSearchForFinish.action?pageNum=1">已收货</a>
                        <span>${today }</span>
                        <c:if test="${size!=0 }">
                        	<button id="receive" class="btn btn-info">一键接单</button>
                        </c:if>
                    </div>
                    <!-- 如果有订单 -->
                    <c:if test="${size!=0 }">
                        ...
                        <tbody>
                       	<c:forEach items="${orderUserListForPage }" var="i" varStatus="status">
                           	...
                           	<c:if test="${i.isExit!='2' }">
	                            ...
	                            <td>
	                            	<c:if test="${i.isExit=='2' }">
		                                <span><a href="${pageContext.request.contextPath}/management/receiveIntegralOrder.action?id=${i.orderUserId}">接单</a></span>
		                                <span><a class="refuseForIntegralOrder">拒单</a></span>
		                                <input type="hidden" value="${i.orderUserId}" class="integralOrderId"/>
		                          	</c:if>
		                          	<c:if test="${i.isExit!='2' }">
		                                <span><a href="${pageContext.request.contextPath}/management/receiveOrder.action?id=${i.orderUserId}">接单</a></span>
		                                <span><a class="refuse">拒单</a></span>
			                        </c:if>
	                            </td>
...
<script>
		...
		$.ajax({
			...
			success:function(data){
				if(data=='1'){
					...
					window.location.href="${pageContext.request.contextPath}/management/orderSearch.action?pageNum=1";
					console.log(data);
				}
				getTimeOrder();
			}
    ...
    $(function () {
        ...
        $("#receive").click(function (e) {
			  window.location.href="${pageContext.request.contextPath}/management/receiveAllOrder.action";
         });
        
        $(".refuse").click(function(e){
        	var index=$(".refuse").index(this);
        	var orderUserId=$(".orderUserId").eq(index).val();
        	var con=confirm("确定拒绝此单吗？");
 			if(con==true){
 				alert("提交成功")
 				window.location.href="${pageContext.request.contextPath}/management/refuseOrder.action?id="+orderUserId;
 			}else{
 				 e.preventDefault();
 			}
        })
        $(".refuseForIntegralOrder").click(function(e){
        	var index=$(".refuseForIntegralOrder").index(this);
        	var orderUserId=$(".integralOrderId").eq(index).val();
        	var con=confirm("确定拒绝此单吗？");
 			if(con==true){
 				alert("提交成功")
 				window.location.href="${pageContext.request.contextPath}/management/refuseIntegralOrder.action?id="+orderUserId;
 			}else{
 				 e.preventDefault();
 			}
        })
