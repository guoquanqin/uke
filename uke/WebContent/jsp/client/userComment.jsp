...
<link rel="stylesheet" href="${pageContext.request.contextPath}/css/client/userComment.css"/>
    ...
    <div class="container">
            ...
            <div id="userInfo">
                <div id="user">
                    ...
                    <div class="userbox">
                        ...
                        <span><a href="#"><img src="${pageContext.request.contextPath}/images/icon/订单.png">历史订单</a></span>
                        <span><a href="#"><img src="${pageContext.request.contextPath}/images/icon/用户a.png">用户中心</a></span>
                    </div>
                </div>
            </div>
            <div id="list">
        ...
        <div class="rightsidebar">
            ...
            <ul>
                ...
                <li class="comment">
                    <div class="commentTop"><fmt:formatDate  pattern="yyyy-MM-dd HH:mm:ss" value="${commentList.get(status.index).commentTime }"/> &emsp;&emsp;订单号：${i.orderUserId }</div>
                    <div class="commentContent">
                        <div class="leftContent">
                        <c:forEach items="${i.orderGoodsList }" var="j">
                            <span><a href="${pageContext.request.contextPath}/client/simpleGoods/goodsDetail.action?id=${j.goods.goodsId}"><img src="${pageContext.request.contextPath}/images/${j.goodsType.gtName }/${j.goods.goodsPicture}"></a>&emsp;&emsp;${j.goods.goodsName }</span>
                        </c:forEach>
                        </div>
                        <div class="centerContent">
                           		${commentList.get(status.index).commentContents }
                            <span>
                            <c:forTokens items="${commentList.get(status.index).commentPhoto }" var="i" delims="/">
                            <img src="${pageContext.request.contextPath}/images/评价图片/${i}">
                            </c:forTokens>
                            </span>
                        </div>
                        <div class="rightContent">
                        ${fbCommentListForPage.get(status.index).feenbackComment.feedbackContent }
                        </div>
                    </div>
                </li>
                </c:forEach>
            ...
            <c:if test="${size>7 }">
	            <ul class="pagination">
	                ...
	                <li><a href="${pageContext.request.contextPath}/client/userCenter/userComment.action?pageNum=1">首页</a> </li>
	                <c:if test="${pageBean.pageNum==1 }">
		                <c:forEach  begin="${pageBean.start }" end="${pageBean.end }" step="1" var="i">
		                	<li><a href="${pageContext.request.contextPath}/client/userCenter/userComment.action?pageNum=${i}">${i }</a> </li>
		                </c:forEach>
		                <li><a href="${pageContext.request.contextPath}/client/userCenter/userComment.action?pageNum=${pageBean.pageNum+1}">下一页</a> </li>
	                </c:if>
	                 <c:if test="${pageBean.pageNum>1&&pageBean.pageNum<pageBean.totalPage }">
	                 	<li><a href="${pageContext.request.contextPath}/client/userCenter/userComment.action?pageNum=${pageBean.pageNum-1}">上一页</a> </li>
		                <c:forEach  begin="${pageBean.start }" end="${pageBean.end }" step="1" var="i">
		                	<li><a href="${pageContext.request.contextPath}/client/userCenter/userComment.action?pageNum=${i}">${i }</a> </li>
		                </c:forEach>
		                <li><a href="${pageContext.request.contextPath}/client/userCenter/userComment.action?pageNum=${pageBean.pageNum+1}">下一页</a> </li>
