        ...
        <div class="order">
            ...
            <div class="orderDetail">
                <div class="goods">
                    ...
                    <label>
                    <a href="${pageContext.request.contextPath}/client/simpleGoods/goodsDetail.action?id=${goodsList.goodsId}"><img src="${pageContext.request.contextPath}/images/${goodsType.gtName }/${goodsList.goodsPicture}"></a>${goodsList.goodsName }
                    </label>
                    <label>
                   		<c:if test="${size=='1'}">
                    ...
                    <label class="price">
                    	...
                    	<c:if test="${size=='2'}">
                    		￥${goodsList.goodsMiddlePrice }
                    	</c:if>
                    	<c:if test="${size=='0'}">
                    		￥${goodsList.goodsPrice }
                    	</c:if>
                    	<input type="hidden" class="form-control" value="${goodsList.goodsMiddlePrice }" class="goodsMiddlePrice"/>
                    	<input type="hidden" class="form-control" value="${goodsList.goodsPrice }" class="goodsPrice"/>
                    </label>
                    <label ><input class="form-control" style="width:90px" type="number" class="number"  min="1" value="${number }"/></label>
                    <label><textarea cols="12" class="form-control" rows="2" maxlength="50"></textarea></label>
                    <label class="simplePrice">
	                    <c:if test="${size=='1'}">
	                    		￥${number*goodsList.goodsPrice }
	                    	</c:if>
	                    <c:if test="${size=='2'}">
	                    		￥${number*goodsList.goodsMiddlePrice }
	                    </c:if>
	                    	<c:if test="${size=='0'}">
	                    		￥${number*goodsList.goodsPrice }
	                    </c:if>
                ...
                <form class="form form-inline">
                    ...
                    <label>订单合计（含运费）:
                   		<font id="allPrice">
                   		<c:if test="${size=='1'}">
	                    		￥${number*goodsList.goodsPrice+2 }
	                    	</c:if>
	                    <c:if test="${size=='2'}">
	                    		￥${number*goodsList.goodsMiddlePrice+2 }
	                    </c:if>
	                    	<c:if test="${size=='0'}">
	                    		￥${number*goodsList.goodsPrice+2 }
	                    </c:if>
...
<script>
    ...
    function checkPrice(){
        ...
        var allPrice=parseFloat($("#allPrice").text().substring(1));
        if (allPrice<18){
    ...
    $(function () {
        ...
        $(".size").click(function () {
            ...
            if(size=='大'){
                $(".price").eq(index).text("￥"+$(".goodsPrice").val());
            }
            if(size=='中'){
                $(".price").eq(index).text("￥"+$(".goodsMiddlePrice").val());
            }
        ...
        $(".number").click(function () {
            var allPrice=2;
            var simplePrice=$(".number").val()*$.trim($(".price").text()).substring(1);
            allPrice+=simplePrice;
            $("#allPrice").text("￥"+allPrice);
            $(".simplePrice").text("￥"+simplePrice);
