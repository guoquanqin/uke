...
<title>设置中心-修改密码</title>
<link rel="stylesheet" href="${pageContext.request.contextPath}/js/bootstrap/css/bootstrap.min.css" />
<link rel="stylesheet" href="${pageContext.request.contextPath}/css/client/userSettingPass.css"/>
            ...
            <div class="bottombar">
		        <div class="part4">
	                  ...
	                  <div role="form" class="form form-inline">
	                      <span><div>请输入当前密码</div><input v-model="nowPass" type="password" maxlength="11" name="userPhone" class="form-control" required/><font ref="nowMess" style="color:red;">${mess }</font></span>
	                      <span><div>新密码</div><input type="password" name="password" class="form-control" v-model="newPass" required/><font ref="passMess">请输入8到16位有效字符</font></span>
	                      <span><div>确认密码</div><input type="password" name="password" class="form-control" v-model="newPass2" required/><font ref="passMess2"></font></span>
        ...
        methods: {
            ...
            checkpd(){
                if(this.newPass!=this.newPass2){
                    this.$refs.passMess2.style.color='red';
                    this.$refs.passMess2.innerText='密码不一致，请重新输入';
                    return false;
                }
                else
                {
                    this.$refs.passMess.innerText='';
                    this.$refs.passMess2.innerText='';
                    return true;
                }
            },
            /*检查不能为空 */
            ...
            submit(e){
                var check=true;
                check=check&&this.checkIsNull(this.nowPass,this.$refs.nowMess,"密码不能为空");
                check=check&&this.checkIsNull(this.newPass,this.$refs.passMess,"密码不能为空");

                check=check&&this.checkLength(this.newPass,"密码",8,16,this.$refs.passMess);
                check=check&&this.checkIsNull(this.newPass2,this.$refs.passMess2,"确认密码不能为空");
                check=check&&this.checkpd();
                if(check){
                    alert("提交成功");
                    window.location.href="${pageContext.request.contextPath}/client/userCenter/userUpdatePass.action?nowPass="+this.nowPass+"&newPass="+this.newPass;
                }
                else
                    e.preventDefault();
            }
